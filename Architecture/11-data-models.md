...
A model that no single space owns leaves `spec.contextSpaceRef` out (ADR-N-039, DM-74). Without
`metadata.namespace` it is an **organization model** at `datamodels/{name}/` of the organization
repository, which every project of the organization reads and only red-lane Changes to that
repository edit. With it, it is a **project model** at `projects/{p}/datamodels/{name}/`. A space's
one model uses either kind by `import` at a pinned major (DM-75). A project shares its model upwards
only through a promotion that an organization-scope approver approves (DM-76), and a project bundle
carries copies of the organization models it imports (MF-49, MF-50).
