...
| `context-endpoint-apps` | `/apps/*`, narrowed by `vars` to `^/apps/[^/]+/api/endpoint/` | `{host}` | 35 | `context-gateway:8080`, the `/apps/{name}` prefix stripped | The apps session (`jc_edge_apps` on `/apps/`) becomes the bearer, or a bearer passes through; anonymous stays anonymous | Class 3 and Class 4 | as `context-endpoint-portal`, `proxy-rewrite` `regex_uri` |
...
| `app-{name}` | `/apps/{name}/*` | `{host}` | 30 | `app-{name}` in `{release}-{project}-apps` (service and fullstack) or `portal:8080` (static) | Session of the App's own client `app-{name}`; `unauth_action: pass` for `visibility: public` | Class 1 | `openid-connect`, a cookie of its own on `/apps/{name}/`, logout `/apps/{name}/logout` (AP-26…AP-29, AP-112) |
