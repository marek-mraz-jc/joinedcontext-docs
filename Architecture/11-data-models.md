...
slots:
  ...
  temperature:
    ...
    unit:
      ucum_code: "Cel"
      exact_mappings: ["unece:CEL", "qudt-unit:DEG_C"]
...
The crosswalk from "the author picked microgram per cubic metre" to `GQ`,
`qudt-unit:MicroGM-PER-M3` and `qudt-quantkind:MassConcentration` lives in **one** place, the
editor's unit table, because that is where a person picks a unit. The model then records the
choice, and the generators read the model — a second table in the Rust kinds or in Model Tools
would be a crosswalk that can disagree with the one an author actually used.
