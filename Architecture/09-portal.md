...
| **Organization models** (`models`) | the organization models (DM-74): name, version, lifecycle, classes and the name a project's model imports each by, `org.{name}.v{major}` (DM-75); open to every person with a binding in the organization | nothing: an organization model changes by a Change to the organization repository, red lane, and a project model reaches it by **Share with the organization** from its project (DM-76) |
