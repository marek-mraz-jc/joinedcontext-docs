...
| `JC_HEALTH_DIR` | — | no | The directory the ConfigMap `jc-validation-results` is mounted at (`JC_HEALTH_DIR`; OPS-52): one digest per validation check, read on every request to `/api/v1/organization/health`. |
