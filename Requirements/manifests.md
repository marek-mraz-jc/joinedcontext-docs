...
- **MF-48** [S] — The check MUST resolve every reference a manifest makes to a resource of its own project (MF-13). A reference that the project does not hold yet but that an open change of the same project creates (the change proposing that kind and name) resolves as waiting: the check answers `valid: true`, its verdict carries `waitsOn` with that change's id and a warning finding `resolves once chg-… is approved`, and the proposal records the change it waits on in its merge request. Approving a change MUST be refused with `409` while a change it waits on is not merged. A change whose awaited change was rejected MUST be flagged (its `waitsOn` entry is `Rejected`) and MUST stay unapprovable. A proposal sent after the awaited change was rejected is refused like any reference to nothing. A reference that neither the project nor an open change holds MUST be refused naming the field (owner decision 2026-09-24, T-2235).
...
| MF-48 | [Architecture/06-configuration-as-code.md#2-manifest-envelope--kinds-catalogue-cc-09-cc-12](../Architecture/06-configuration-as-code.md#2-manifest-envelope--kinds-catalogue-cc-09-cc-12) | [Testing/04-configuration-and-pipeline-tests.md#1-manifest-validation](../Testing/04-configuration-and-pipeline-tests.md#1-manifest-validation) |
