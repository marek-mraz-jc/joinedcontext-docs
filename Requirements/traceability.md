...
| **PF-01–PF-106** | Platform Invariants | `platform.md` | Portal API (Rust), Gitea, Reconciler, Artifact store (RustFS) | Conftest Quota Checks, URN Format Linters, DB Cascade Tests |
| **MF-01–MF-50** | Manifest Model, Import/Sync/Download | `manifests.md` | Portal API resource API, `jcctl export/import/sync`, Gitea | `import(export(x))==x` proptests, dry-run plan tests, SyncSource loop tests, conflict-policy tests |
| **EP-01–EP-90** | Endpoints & Parity | `endpoints.md` | Context Gateway, Format Translators, ArcSwap Cache, Portal catalogue | Representation Parity Tests, OGC ATS, STA Sensing Profile Suite |
| **DM-01–DM-79** | Data Models & LinkML Editor | `data-models.md` | Portal UI (LinkML Editor), Model Tools image, jcctl `model`, Context Gateway validation | Regeneration diff gate, example-validates test, JSON-LD expansion test, editor Playwright journey, breaking-change classifier tests |
...
| **PL-01–PL-65** | Pipeline Execution | `pipelines.md` | Bento Streams Runner, K8s CronJob, NetworkPolicies, derived pipelines (WASM/container compute), any input the runner ships, sources, steps and outputs (ADR-N-023), the workbench and validated writes (ADR-N-034), stale-entity expiry | `bento lint`, Golden Transformation Tests, Stream Load Tests |
| **AG-01–AG-95** | Agents & MCP Governance | `agents.md` | Gateway MCP Façade, Portal MCP (`/api/v1/mcp`) over the operation registry, Agent Runner Addon, `jc-agent-proxy` | MCP Spec Conformance Suite, Adversarial Prompt-Injection Tests |
| **AP-01–AP-139** | Apps on Demand | `apps.md` | Portal static host, App reconciler (Endpoint+Policy from dataNeeds), in-cluster build lane Job (ADR-N-026) with SBOM, Agent Runner builder profile, APISIX edge login, per-app Keycloak clients and the composed edge file (ADR-N-030) | dataNeeds⊆grants CI test, host-allowlist bundle scan, CSP/SRI tests, preview-sandbox e2e |
...
| **UI-01–UI-87** | Portal & User Interface | `portal-and-ui.md` | Portal UI (React 19), Portal API (Rust), MapLibre / deck.gl | Playwright E2E, Vitest Unit Tests, Axe WCAG 2.1 AA Audits |
| **TS-01–TS-26** | Testing & Quality | `testing.md` | CI Pipelines, Test Harnesses, Fuzzing Frameworks | Cargo Clippy, Schemathesis, K6 Load Tests, Trivy Scans |
