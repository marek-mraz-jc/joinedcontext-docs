...
3. **The SDK reaches the build offline.** The builder image carries the template's dependencies installed once against the SDK of the same Portal version, and the lane links them into the clone, so a build installs nothing and needs no registry. SDK-12 already forbids every package beyond the template's, so nothing else can be asked for. This answers the question T-0682 asked ("where CI installs the SDK"). Amended 2026-09-22 (T-2592): the first text had the lane run `pnpm install --offline --frozen-lockfile`. A generated application carries no lockfile, and an install runs the scripts of the packages it installs. Linking needs neither.
