...
| **AR-2** | Deprecate Insecure Client Tech | Zero Flash/Java-applets/ActiveX; the Portal ships one HTML5 bundle and no plugin runtime. The edge sets HSTS, `X-Frame-Options`, `X-Content-Type-Options: nosniff` and `Referrer-Policy`. The Portal and each App send their own `Content-Security-Policy`; the edge adds a baseline one (`frame-ancestors`, `object-src 'none'`, `base-uri 'self'`) only to an answer that carries none (OPS-34), so a forge or dashboard page that sets no policy still restricts plugins and framing but not its script origins. | **PARTIAL** |
...
The pipeline runner's streams API (4195, `PUT` and `DELETE /streams/{project}.{name}`) holds the pipeline
