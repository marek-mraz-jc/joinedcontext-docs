...
- **Use.** A space still has exactly one model (DM-61). That model uses a shared one by `import` at a pinned major, named `org/{name}/v{major}` or `project/{name}/v{major}` (import-map keys with no colon, so LinkML never expands them as CURIEs): an organization model or a project model of its own project, never another project's. The Portal resolves the import from the repository at that version and hands Model Tools the imported source beside the model, because Model Tools reads no platform state (Architecture/11 §6.5). An organization model that a space imports cannot be deleted. The refusal names the importers, and `retired` (DM-26) is the way out.
