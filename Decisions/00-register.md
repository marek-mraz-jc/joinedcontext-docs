...
| [ADR-N-025](adr-n-025-one-mcp-connector-several-endpoints.md) | One MCP Connector Over Several Endpoints | **Accepted** | 2026-09-25 | A hub at `/api/mcp` on the gateway lists the Endpoints a token may read and offers one catalogue whose every tool takes a required `endpoint`; the client remembers its preferred Endpoint, the server remembers nothing; each call runs as that Endpoint's own call; the connect-time `endpoint:{slug}` scopes bound what a hub token reaches; the per-Endpoint URL stays. |
...
| [ADR-N-039](adr-n-039-organization-and-project-data-models.md) | Data Models at Organization and Project Level, Shared Upwards Only by an Administrator | **Accepted** | 2026-09-25 | A model lives in the organization repository (every project reads it, red-lane edits) or in a project; a project shares a model upwards only by a red-lane Change an organization-scope approver approves; spaces import a pinned major; a project bundle carries copies of the organization models it uses, and an import maps them onto an identical organization model or lands them as project models, never overwriting. |
...
- [ADR-N-025](adr-n-025-one-mcp-connector-several-endpoints.md) — referenced above.
...
- [ADR-N-039](adr-n-039-organization-and-project-data-models.md) — referenced above.
