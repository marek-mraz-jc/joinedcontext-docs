...
Trade*, at a pinned revision whose URL, date and SHA-256 are recorded beside it, with the
Recommendation 21 codes the platform needs (`C62`, "one", among them). It is vendored once, in
the platform repository, as `crates/jc-core/data/unece-rec20.json`, generated by a script from
the official list joined with QUDT's own `qudt:uneceCommonCode`, `qudt:ucumCode`,
`qudt:hasQuantityKind`, `qudt:conversionMultiplier` and `qudt:conversionOffset`. Nobody maps a
code by hand. One entry is:

```json
{ "code": "GQ", "name": "microgram per cubic metre", "symbol": "µg/m³", "ucum": "ug/m3",
  "qudt": "MicroGM-PER-M3", "quantityKind": "MassDensity",
  "factor": 1e-9, "offset": 0, "frequent": true }
```

`factor` and `offset` convert a value to the SI base unit of its quantity kind
(`si = value × factor + offset`). `frequent` marks the municipal set a picker shows first
(µg/m³, °C, %, km/h, kWh, …). Each repository carries a **generated** copy, never an edited
one: the Portal UI, Model Tools and the SDK. A check in each
repository's CI fails when its copy differs from the source. Nothing is fetched at runtime.
