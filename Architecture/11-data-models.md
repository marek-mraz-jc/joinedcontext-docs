...
| `POST /generate` | `{"source", "imports"?}` | the artifact set of one LinkML document; `imports` maps each `org.{name}.v{major}` or `project.{name}.v{major}` the document imports, directly or through another import, to that model's LinkML (at most 32, DM-75), and an import left out is an `errors` entry naming it, never a file Model Tools looks for |
