...
- **EP-24** [A][S] — Each Endpoint MUST serve an isolated MCP instance at `/api/endpoint/{endpointSlug}/mcp` conforming to SP-14 through SP-20, whatever its audience and whether or not `spec.enabledRepresentations` lists `mcp`; only `spec.mcp: false` in its manifest turns the instance off, and the path then answers as an unknown slug does (EP-23). The instance adds no right: every call is the Endpoint's own Policy decision with the caller's token (EP-25, SP-16), and a caller that may not reach the Endpoint's data may not reach its MCP. `spec.mcp: false` beside `mcp` in `spec.enabledRepresentations` MUST be refused as a contradiction.
...
- **EP-55** [A][S] — Every Endpoint and every space MUST answer `GET …/access` with the caller's effective grant document: the Policy set the Endpoint evaluates (its space's, narrowed by `policyRef` or, on an App's generated Endpoint, by AP-139) partially evaluated with subject, endpoint and time known and the resource unknown, produced by the same in-process PDP that enforces requests (EP-06).
