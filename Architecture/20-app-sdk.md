...
**Design tokens.** `src/design-tokens.json` is the one place the look lives. At start the SDK turns it into CSS variables (`--jc-color-accent`, `--jc-font-body`, `--jc-space-2`, …), registers an ECharts theme and a Recharts palette from its chart colours, and hands the map its point and layer colours, so a person or the model re-themes the whole application by editing one file (SDK-25). At the start of a run the Portal writes the file itself, generated from the installation's branding: one of 135 looks (which brand colour leads the accent and how far its hue turns, sharp, soft or round corners, a compact, regular or airy density), the one no other App of the project holds, with every colour pushed until text reads at 4.5:1 and a chart mark at 3:1. The model keeps it and makes the App its own through layout, composition and copy, so two Apps over the same endpoint differ before the model writes a line (AP-123).

**Colour, a chart and a map on every published App (AP-138).** A published App is a React App on the SDK wearing its tokens: the overview a person lands on shows the data in a chart, and data with a location on a map over the project's basemap, its marks coloured by a value with a legend beside it. The template starts that way (the overview charts every type and maps every located one), so a generated App meets the rule on its first build; the model may replace a chart or the map with a better one and never removes the last. A screen of number tiles, a plain list or unstyled text is what the App probe (AP-136) records as a fail. Plain-HTML samples stay in the SDK documentation as teaching examples only.

**No two Apps alike (AP-137).** The look is one of three things that make an App its own; the gallery sample the request is closest to (SDK-24) brings the other two, its `archetype` and its `layout` (`hero-map`, `tabs-with-detail`, …, as each `sample.json` declares). Before the first call the run asks the same gallery about the latest request of every other App of the project. When one of them already has this archetype and this layout, the conversation says so by name, and the model is told to lay this App out as a gallery layout no App of the project holds yet and to say in one sentence how it differs. It is advice, not a gate: the next message may ask for the same look, and gets it. Nothing is screenshotted; the comparison is of choices, which the model can act on.
...
- **The pack.** Every file of the template (interface, functions, tests, `package.json`), the SDK's `API.md` and type declarations, `jc-types.ts` (rendered from the model of every endpoint that serves a needed type), the confirmed data needs, the endpoint's effective access, five entities per type and the prompt. A run over several endpoints adds one section listing each endpoint's name, context space and served types; its samples are read through each endpoint that serves the type (`/v1/data/endpoints/{slug}/…` for all but the primary, [19 §4](19-agent-runner.md#4-the-proxy-surface)); a type two endpoints of one space serve (an operations and a public endpoint) is sampled through both, the second asked for the ids the first answered, and the rows joined by `id`, with a line naming which endpoint carries which attributes, so the application reads and joins them the same way. When the prompt names the keywords of a gallery sample (`sdk/samples/<name>/sample.json`: a map for residents, an operations desk, a KPI dashboard, a calendar, a form-first workflow, a data story, a real-time monitor), the pack also carries that sample's README and source, not its tests, fixtures or look, as the example to adapt; a call that repairs a build or a preview does not (T-2778). Nothing the model needs to write correct code is left for it to guess. Everything but the prompt, what it selects and the samples is the same for every run of one Portal version and endpoint, so the provider's prompt cache pays for it.
...
- **Verification.** A version on screen is not a finished one. Once it renders, the SDK walks the application's navigation, reads every page and posts a `jc-observation` (§5). The Portal checks it with no model call: runtime errors and failed function calls since the version, failed requests, `NaN`, `undefined`, `[object Object]` or `Invalid Date` on a page, and, per entity type of the data needs, whether any of the five sampled entities appears by name or local id. Problems go back to the model as a verification pass with the observation, at most three after each instruction: a version of the first run is repaired by one more pass over the project, a version an instruction produced by the editing agent of §4.2 with the problems as its instruction, so an application that exists is never written whole again; a clean check is one line on the conversation ("Checked 4 pages: 5 of 5 stations shown, no errors"). A person's message comes before a pending pass (SDK-27, SDK-28).
- **Tests.** A version that builds and holds tests (`*.test.ts`, `*.test.tsx` under `src/` or `functions/`) is tested before the run offers publication, the way the build lane tests it (SDK-38). The Portal writes the version's files into one `ConfigMap` and starts a `Job` in the installation's app-tests namespace on the lane's own `joinedcontext-app-builder` image: it links the release's installed dependencies into the project, runs `vitest` once with the lane's configuration and prints one result line, `JC-TESTS {passed, failed, failures: [{file, name, message}]}`, which the Portal reads from the pod's log. The pod holds no service-account token and no secret, the namespace admits no traffic in or out (the SDK's `stubClient` answers every data call inside the test), it runs under the restricted Pod Security Standard with a read-only root and a `/tmp` of its own, and the `Job` stops at 1 CPU, 1 GiB and 120 seconds. The Portal removes both objects when it has the result. The result is a `tests` event (API/04 §4). Failing tests go back to the model with each failure's file, test name and message, as one more verification pass under the same limit of three, handled like the others: by a pass over the project in the first run, by the editing agent after an instruction. `publish` answers `409` while the newest version's tests fail or still run. Without the namespace (a Portal outside a cluster) the run says the tests were not run and the build lane's check stays the only one (SDK-24).
...
The agent stops at `finish` or at the profile's step limit (AG-25), and at the ceiling of one instruction: 12 model calls and 500,000 input tokens. A turn that reaches it says so on the conversation with the calls and tokens it spent; what passes the check is published, the rest stays as it was. The loop keeps each call small (SDK-20):

- **Grounded once.** The opening message carries every application file whole while they fit in 200 kB, smallest first; a `read_file` of a file shown there and unchanged since answers that it is already in the message.
- **Folded history.** A tool result of more than a few lines is sent whole to the next two model calls, then as its first line and its size; the model reads a file again when it needs it. A long argument of an earlier call (a file written whole, a large replacement) is folded the same way.
- **Several calls an answer.** The model is asked to read what it needs in one answer and make every edit of the request in the next, then `check`, instead of one tool call per round trip.
- **Visible cost.** Every model call is a `usage` event with its input and output tokens (API/04 §4), so a run's cost per call is on its record.

Every tool call is a run event on the conversation; the preview reloads when a `check` passes after a change, so the person watches the application change while the agent works. Paths and imports outside SDK-11 and SDK-12 are refused as tool errors, never applied.

**Commits.** The first run and each finished agent turn commit the files they changed to the run branch (SDK-17). The application's tests run in the run's sandbox before publication is offered (SDK-38, §4.1) and again in the build lane at publication (SDK-24).
...
2. The installation's `gitea-runner` picks the job up: `act_runner` in host mode on the `joinedcontext-app-builder` image of the Portal's own release, which carries the template's dependencies installed once against the release's own packed SDK and links them into the checkout: the workflow installs nothing, so no install script an application ships ever runs, and a `package.json` naming another package fails the build (AP-81, AP-82, SDK-12).
3. The workflow runs the interface and function tests, builds, bundles `functions/*.ts` into `functions.js`, computes `integrity.json` (AP-12) and the SBOM (AP-11), and publishes `bundle.tar.gz` and `sbom.cdx.json` as the generic package `app-{name}`, version `{commit}-{first 12 hex of the digest}`, so a rebuild on a newer platform release is a version of its own (AP-101).
4. Its last step proposes `status.build {digest, commit, sdkVersion, builtAt}` through the Portal API with the lane's token, an Actions organization secret whose only right is that one field and which no step running application code sees (AP-73, AP-80). The Portal refuses a commit that is not on the default branch or a package that does not match (AP-104).
5. The static host fetches `app-{name}@{commit}-{digest12}` read-only (or the bare `{commit}` of a build published before T-2671), checks the digest before it serves a byte, and keeps it under `{apps_cache_dir}/{name}/{hex}/` (AP-102). A failed run, or a package that does not match, leaves the previous build serving (AP-72).

The workflow file is the Portal's: a run commit that touches `.gitea/` is refused, so what runs is only what a reviewer merged (AP-100). The runner executes untrusted application code, so it holds no Kubernetes token and no container socket, runs under the restricted Pod Security Standard with egress to the forge and the Portal API only, and wipes its work directory after every job (AP-81). An App's `build` job runs in a build pod of that App alone: the Portal starts one `Job` per queued build, registered with a token of that repository only, which takes that one job and ends, and which mounts the App's own build cache `build-cache-{app}` so a rebuild compiles only what changed (AP-130, AP-131, ADR-N-028 §5). The catalog reads the repository's workflow runs: `building`, `build failed` with a link to the run's log, or `running` with the commit in its tooltip, and offers **Open** only while a build is served and never on a retired App (AP-86). The App page links the repository, the latest run and the package (AP-103).

A `static` application has one of three shapes, told apart by `spec.build`:

| `spec.build` | Shape | What the lane does |
|---|---|---|
| `{ node: "22" }` | a Vite project: React on the SDK, what the generator writes | links the installed dependencies, the tests, `pnpm build` |
...
A published `static` App therefore names `spec.source.git`, and a proposal or an import that publishes one naming `spec.source.path` is refused, as `jcctl validate` refuses it (AP-87): the lane has nothing to build from a folder of the configuration repository, and such an App would answer `404` to everyone it was published for. Applications created before one repository per application existed keep their manifest; they are retired, or published again from a run in their own repository. The reference applications whose bundle the Portal image ships carry `joinedcontext.com/shipped-with: portal`, which the Portal accepts only when its static host holds that bundle.
