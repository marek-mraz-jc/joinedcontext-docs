...
- **AP-82** — The builder image MUST be tagged, signed and pinned by digest with the Portal release it belongs to, and MUST bake that release's SDK and the template's dependencies already installed, so the lane runs no install step, runs no package install script and reaches no package registry; a `package.json` that names a package the template does not install MUST fail the build (SDK-12).
