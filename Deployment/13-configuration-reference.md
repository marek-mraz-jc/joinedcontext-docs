...
| `JC_INTERNAL_BIND` | `0.0.0.0:9090` | no | `JC_AGENTS_NAMESPACE` and `JC_AGENT_PROXY_BASE` are set together or not at all; `JC_PORTAL_NAMESPACE` (default: the workspaces' own namespace), `JC_INTERNAL_BIND` (default `0.0.0.0:9090`) and `JC_AGENT_RUN_TTL` (whole seconds, default `1200`) tune the rest. |
...
| `JC_PORTAL_BUILD_CACHE_SIZE` | `1Gi` | no | Where an App's build pods run and on which runner images (`JC_PORTAL_BUILD_NAMESPACE` with `JC_PORTAL_BUILD_IMAGE_NODE`, `JC_PORTAL_BUILD_IMAGE_RUST` and the forge's API base; `JC_PORTAL_BUILD_CACHE_SIZE`, default `1Gi`; AP-124, AP-125). |
| `JC_PORTAL_BUILD_IMAGE_NODE` | — | no | Where an App's build pods run and on which runner images (`JC_PORTAL_BUILD_NAMESPACE` with `JC_PORTAL_BUILD_IMAGE_NODE`, `JC_PORTAL_BUILD_IMAGE_RUST` and the forge's API base; `JC_PORTAL_BUILD_CACHE_SIZE`, default `1Gi`; AP-124, AP-125). |
| `JC_PORTAL_BUILD_IMAGE_RUST` | — | no | Where an App's build pods run and on which runner images (`JC_PORTAL_BUILD_NAMESPACE` with `JC_PORTAL_BUILD_IMAGE_NODE`, `JC_PORTAL_BUILD_IMAGE_RUST` and the forge's API base; `JC_PORTAL_BUILD_CACHE_SIZE`, default `1Gi`; AP-124, AP-125). |
| `JC_PORTAL_BUILD_NAMESPACE` | — | no | Where build pods run (AP-124): nothing when `JC_PORTAL_BUILD_NAMESPACE` is unset, and every other part required once it is, because a namespace with no image would start pods that never run. |
...
| `JC_PORTAL_NAMESPACE` | — | no | `JC_AGENTS_NAMESPACE` and `JC_AGENT_PROXY_BASE` are set together or not at all; `JC_PORTAL_NAMESPACE` (default: the workspaces' own namespace), `JC_INTERNAL_BIND` (default `0.0.0.0:9090`) and `JC_AGENT_RUN_TTL` (whole seconds, default `1200`) tune the rest. |
