...
| `context-space-read`, `context-space-portal-read` | `/cs/*`, `GET` and `HEAD` | `{host}`; `portal.{host}` | 16 | `context-gateway:8080` | as `context-space` and `context-space-portal`, whose plugin configs they share | Class 2, a bucket of its own for reads | a route-level `limit-count` that replaces the shared one |
...
| `context-mcp-hub-portal` | `/api/mcp*` | `portal.{host}` | 20 | `context-gateway:8080` | Edge session becomes the bearer, or a bearer passes through; the URL the Endpoints view shows | Class 3 and Class 4 | the `context-endpoint-portal` plugin config |
...
3. **`limit-count` and `limit-conn` (tiered rate limiting):** counted per APISIX worker with `policy: local`, so a gateway with several replicas allows the ceiling per replica. Every limit answers `429` and sets the quota headers. These classes are anti-flood protection for the node, sized far above what one caller needs; they are not an Endpoint's rate limit, which exists only when the Endpoint's manifest sets one (EP-20). Whether the Class 3 and Class 4 buckets stay on the `context-endpoint` routes is the owner's decision recorded in T-2775; until then they stay.
   ...
   - **The Organization sets the counts (ADR-N-035).** A limit the Organization tunes carries the label `jc-rate-class: <class>` on its plugin config or route (`rateClass:` in the component file), and the Portal writes the class's rate into its `count` each time it composes the served file (section 3): `spec.limits.edge.requestsPerMinute.web` on `portal-ui`, `portal-public`, `apps-surface` and `portal-redirect`, `.api` on `portal-api`, `.publicEndpoint` on the `context-endpoint` routes, `.dataRead` on `context-space-read` and `context-space-portal-read`, and `.dataWrite` on `context-space` and `context-space-portal`, which after the read routes carry every other method. Unset, a class counts the catalog default, the number the component file holds; a value past the operator's bound (`portal.organizationBounds`) counts the bound. A chain without the label keeps the number helm renders. The largest request body works the same way: the global rule `edge-body` (`client-control`, `max_body_size`) holds every request to `spec.limits.edge.maxRequestBodyMegabytes`, 16 MiB unset and at most 64, and the Portal writes it with the counts. Per request it replaces the server block's `client_max_body_size 16m` in either direction, for a body with a `Content-Length` and a chunked one alike; the server block's number holds only while no rule is served.
...
9. **`serverless-post-function` (baseline Content Security Policy, OPS-34):** runs in the `header_filter` phase on every route and sets `Content-Security-Policy` only when the answer carries none. The Portal sends its own policy and each App its per-App one (AP-12, AP-122): the static host writes it for a `ui` App, and the reconciler sets it on a `ui-rust` App's `app-{name}` route with `response-rewrite` `headers.set` from the App's manifest, in place of the `frame-ancestors` a `ui` App's route adds; `response-rewrite` would replace them, and a second header would intersect with them and break the page, so the edge never touches a policy an upstream wrote. The baseline is `frame-ancestors` matching the route's `X-Frame-Options` (`'self'` or `'none'`), `object-src 'none'` and `base-uri 'self'`: it restricts framing, plugins and `<base>` on a page that sets no policy (Gitea, Grafana, CKAN) without restricting its script origins, which only the upstream knows. The phase runs for the answers APISIX writes itself (a `401` of `key-auth`, the `302` of `openid-connect`), so those carry it too; the `429` page of the server block's `error_page` is built after the plugin chain and sets the `DENY` baseline itself.
