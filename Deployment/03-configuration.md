...
| `JC_GATEWAY_DELIVERY_KEY` | unset, and then every subscription write is refused with `501` | secret, at least 32 bytes: the key the gateway signs the subscriber stored with each subscription with, and verifies at every delivery before deciding it again ([GW27](../Requirements/gateway-firewall.md)); generated by the deployment, and rotating it stops every stored subscription until it is written again |
