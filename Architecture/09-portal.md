...
| RoleBinding, Role, ServiceAccount, AgentProfile | Access page: who holds which role (Grant a role, Edit, Remove), effective permissions, API keys, agent access; Organization → Agent profiles creates and edits an AgentProfile through a form (T-1536) | list, get, create, update, delete | `jc_resource_*` | `grant_role`, `change_resource` |
| SyncSource, CkanInstance, Blueprint, Mapping, ModelProjection, ScopeDefinition, SharedSpaceReference, ContextSourceRegistration, Layer, UiSchema, Bundle, DataSpaceParticipant, DataOffer, DataAgreement, Project | a page for sync sources, CKAN and flows; a form on the list for DataAgreement, DataOffer, ModelProjection and ScopeDefinition (T-1542, T-1543, T-1548, T-2955), and on the Organization page for Blueprint, DataSpaceParticipant and Environment (T-1537, T-1544, T-1545); a generic list for the rest | list, get, create, update, delete | none | none |
...
| **Agent profiles** (`agentprofiles`) | the `AgentProfile` manifests of namespace `org`: what an assistant or builder run executes and may reach (AG-26, AG-47…AG-50, AG-70): role, image digest, model, limits, hosts, tools, access | **New AgentProfile** or a change to one through its form; no field takes a model key, the proxy holds those (T-1536) |
| **Data space** (`dataspaceparticipants`) | the organization's one `DataSpaceParticipant` (DS-04, DS-07): its did:web identifier, credential issuer, connector address and engine, trusted issuers | **New DataSpaceParticipant** while there is none, else Edit of the one there is: jc-core files it at one path whatever its name (T-1544) |
| **Environments** (`environments`) | the `Environment` overlays of namespace `org` (CC-73…CC-75): the organization's domain in each, hosts and image digests by component, the secret backend, feature flags | **New Environment** or a change to one through its form: each map as rows, a digest never a tag, a backend never a secret (T-1545) |
| **Organization models** (`models`) | the organization models (DM-75): name, version, lifecycle, classes and the name a project's model imports each by, `org.{name}.v{major}` (DM-76); open to every person with a binding in the organization | nothing: an organization model changes by a Change to the organization repository, red lane, and a project model reaches it by **Share with the organization** from its project (DM-77) |
...
| **All applications** (`applications`) | every App of every project: title, project, lifecycle, what it builds from | **Export** of one App, its repository as a git bundle beside its manifest; **Import app** into a project under a name, the red-lane `Change` that adds it ([API/01 §10](../API/01-portal-api.md)) |
| **Endpoints** (`endpoints`) | every Endpoint of every project in one table: its project and space, its audience, and one link per representation; each project opens that project's own Endpoints. For an administrator of the organization only (`approve` and `delete` on `RoleBinding` at organization scope, PF-03, PF-61): anybody else meets the page's notice of UI-75 there as on every tab, and `GET /api/v1/endpoints` answers them `404`. `/endpoints` opens this tab; the project menu has no entry for it | nothing |
