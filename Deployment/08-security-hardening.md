...
| **CHK-15** | Production is not dev | `cd "${JC_DEPLOYMENT_DIR:-../joinedcontext-deployment}" && JC_PRODUCTION_ENVIRONMENT=production python3 -m pytest -q tests/test_production_is_not_dev.py` | Every case passes: the render names its own `domain` and `orgDomain`, and carries no demo person, demo city account, demo feed, wildcard-DNS dev host, `dev` namespace or realm, debug switch, open CORS, verbose error or written model key. The render itself stops on a placeholder domain or an unnamed runner project. | CC-73, CC-75 |
