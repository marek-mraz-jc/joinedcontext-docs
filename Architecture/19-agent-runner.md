...
| `/v1/data/{*rest}` | any | `http://context-gateway/api/endpoint/{run.slug}/{rest}` | The run's delegated token: the starting person's, exchanged for the proxy's client (ADR-N-038, AG-94) | Path traversal (`..`); write methods (`POST`, `PATCH`, `PUT`, `DELETE`) on read-only runs; client-supplied `NGSILD-Tenant` or slug headers. |
| `/v1/data/endpoints/{slug}/{*rest}` | any | `http://context-gateway/api/endpoint/{slug}/{rest}` | The run's delegated token (ADR-N-038, AG-94) | A `slug` outside the run context's `endpointSlugs` (every endpoint of the run, the primary first, AP-44); otherwise the rules of `/v1/data/{*rest}`. |
| `/v1/data/mcp` | `POST` | `http://context-gateway/api/endpoint/{run.slug}/mcp` | The run's delegated token (ADR-N-038, AG-94) | Requests invoking mutation tools (`upsert_entity`, `create_subscription`) without declared write rights. |
| `/v1/forge/{*rest}` | any | In-Cluster Gitea API | A forge token scoped to the one repository the run works in, issued per request (AG-86) | A repository other than the run's: in layout 2 the run's project repository, or the application's own (AP-72); the organization repository on any run; branch not matching `agent/app-{name}/{runId}`; file path outside `apps/{name}/` of the project (`projects/{project}/apps/{name}/` in layout 1); direct writes to default branch. |
...
| `/internal/runs/{run}/identity` | `POST` | The realm's token endpoint (RFC 8693 exchange), after introspecting both tokens | The proxy's own client credentials, to the realm only | Any caller but the Portal's service account (`JC_PORTAL_CLIENT_ID`, AG-52): `401`, judged before the run is looked up; a token that is not an active token of the run's `createdBy`, or a refused exchange: `403`; a finished run: `409`. The body is `{"subjectToken": …}`, the token is never logged, and the grant is held in memory until the run ends, then revoked (ADR-N-038, AG-94). |
...
- **Who Read the Data**: a run's data-plane calls carry a token exchanged from its person's, so the gateway decides them on that person's grants and its decision log names the person beside the proxy's account (ADR-N-038, AG-94, AG-95). A run that ends revokes the grant; a person who signs out ends their runs' data access.
