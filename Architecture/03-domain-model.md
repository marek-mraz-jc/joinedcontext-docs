...
Every other organization-wide choice and number sits beside `spec.projects` in the same manifest: `spec.policies` (whether public apps are allowed, the password policy, the models the assistant may use) and `spec.limits` (the edge's request rates per class, request bodies, session lifetimes, invitation lifetime, model spend, pipeline samples and uploads). Each entry has a default, a bound the operator sets in the deployment, and one enforcer that reads it: the Portal's verdict, the edge, the gateway, the Keycloak realm, the agent proxy or the runner. Organization settings edit them as one red-lane change an `org-admin` approves. A lowered limit refuses what comes next and deletes nothing, and a refusal names the limit and who can change it ([ADR-N-035](../Decisions/adr-n-035-organization-policies-and-limits.md), PF-96…PF-103).
