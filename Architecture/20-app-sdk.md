...
A published `static` App therefore names `spec.source.git`, and a proposal or an import that publishes one naming `spec.source.path` is refused, as `jcctl validate` refuses it (AP-80): the lane has nothing to build from a folder of the configuration repository, and such an App would answer `404` to everyone it was published for. Applications created before one repository per application existed keep their manifest; they are retired, or published again from a run in their own repository. The reference applications whose bundle the Portal image ships carry `joinedcontext.com/shipped-with: portal`, which the Portal accepts only when its static host holds that bundle.
