...
- Each data need belongs to the first listed endpoint whose `contextSpaceRef` names the need's context space, or to the primary endpoint when none does; its attributes are checked against that endpoint's projection. Its operations are checked against the caller's own grant on that endpoint (AP-44, AP-132): the Portal reads the endpoint's `/access` document from the context gateway with the caller's token, and a need naming an operation the caller does not hold on one of its types answers `400 Bad Request` with each such operation in `errors`, beside every other violation. A Portal cookie session carries no token, so a request from it with a write need answers `400` naming it, and a gateway that does not answer is `503 Service Unavailable`.
...
- Write operations (every operation jc-core's `OperationRef::is_write` names: `createEntity`, `updateAttrs`, `appendAttrs`, `deleteAttrs`, the batch writes, …) trigger `allowsWrite: true` and sandbox provisioning; unauthorized write attempts fail with `403 Forbidden`.
...
id: 19
event: usage
data: {"seq":19,"step":22,"tokensThisStep":4120,"inputTokens":3980,"outputTokens":140,"cachedTokens":3200,"latencyMs":1830,"model":"google/gemini-3.8-flash","reasoningEffort":"medium","cumulativeTokens":382100,"timestamp":"2026-09-25T08:14:03.412Z"}
...
The kinds a stream carries are `status`, `question`, `answer`, `message`, `thought`, `partial`, `tool`, `commit`, `preview`, `preview_error`, `tests`, `usage`, `navigate`, `path` and `lag`. A `tests` event is one run of the application's tests in the run's sandbox (SDK-38): `{version, outcome, passed, failed, durationMs, failures}`, `outcome` one of `running`, `passed`, `failed`, `error` (the sandbox could not run them: a timeout, no pod, an unreadable result, with `reason`) or `skipped` (no sandbox in this installation, or a version without tests, with `reason`), and `failures` at most 20 of `{file, name, message}`, each message cut at 2,000 characters. A `partial` is the assistant's words while the model still writes them (ADR-N-032, T-2821): `{text, elapsedMs}`, `text` the prose of the answer so far, cut before the first tool fence, so a tool call's JSON never shows; at most two a second, each carrying the whole text so far. The `thought` or the step that follows is the answer, and a client draws a `partial` only until an event of another kind arrives after it. A `thought` carrying `failed: true` is a turn that ended without an answer: its `text` says why in words a person can act on (the model service was busy, did not answer within the turn's time, answered nothing readable), never the provider's body, the key or the prompt; the model call behind it was already asked once more after a 408, 425, 429, 500, 502, 503 or 504 or a connection that failed, so the person reads a failure only when the second try failed too. The dock offers Try again on it, which sends the person's last message again in the same conversation (T-2772). A `path` event names the path a conversation is on and who chose it (section 8, [Paths](#paths)). Two of them are written by a person rather than by the agent: `answer` is one question answered, `message` is a free instruction sent between questions (section 5). A `usage` frame is one model call: `tokensThisStep` what it cost, `inputTokens` and `outputTokens` its two halves when the provider reports them, `cachedTokens` the input tokens the provider read from its prompt cache, `latencyMs` the milliseconds from the call leaving `jc-agent-proxy` to its last byte, `model` the model named in the call, `reasoningEffort` the effort it was sent with (`low`, `medium` or `high`: the profile's `model.reasoningEffort`, or the one the call's body named itself) and absent when it named none (T-2998), `cumulativeTokens` the run's total; `jc-agent-proxy` writes one for every call it relays (AG-72, T-2771), and the editing agent one per step of its own (SDK-20). A `status` frame that ends a run by a cancel or an expiry carries `reason`: who cancelled it ("cancelled by jana.kovacova"), or what the expiry lost, and the conversation says it under the ended chat (T-2772). A run that reaches `awaiting_approval` waits on the approval's lease, `JC_AGENT_APPROVAL_TTL` seconds (seven days by default), in place of what was left of its wall clock: an approver has days. When that lease runs out too, the run expires with a `reason` saying its change stays open and that approving it still publishes the application. Every frame's data carries `timestamp`, the RFC 3339 instant the Portal recorded the event (a `status` frame's own `timestamp` is that instant too), so a client reads where a run's seconds went from the stream alone (T-2771).
...
A run whose newest version's tests failed, or are still running in the sandbox, is not published: `409 Conflict` with a `detail` naming the version and up to five failing tests (SDK-38). A version whose tests were skipped because the installation has no sandbox publishes, and the build lane's run is then the check (SDK-24).
