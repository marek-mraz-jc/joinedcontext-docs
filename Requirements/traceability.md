...
| **AP-01–AP-104** | Apps on Demand | `apps.md` | Portal static host, App reconciler (Endpoint+Policy from dataNeeds), in-cluster build lane Job (ADR-N-026) with SBOM, Agent Runner builder profile, APISIX edge login | dataNeeds⊆grants CI test, host-allowlist bundle scan, CSP/SRI tests, preview-sandbox e2e |
