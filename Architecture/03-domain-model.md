...
Organizations declare the domain once (`Organization.spec.domain`); the reconciler verifies ownership by DNS TXT record or by the served `did:web` document before any space of that organisation can accept writes (PF-41). The Portal's reconciler records and reports the verification; the gate that refuses writes is not built yet (T-2377):
