...
- Network: the store is reachable only inside the mesh (Linkerd mTLS, default-deny NetworkPolicy); it has no ingress route. Only the Portal reaches the admin API on port 9000.
...
- **Backup**: low priority (OPS-09 scope) because every object is reproducible: `jcctl artifacts rebuild --org {o}` re-renders schemas, recompiles mappings and re-exports from Git; dumps are the exception and are mirrored to the CNPG backup bucket if retention matters.
