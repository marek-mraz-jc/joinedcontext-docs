...
flowchart TD
    subgraph K8s["Project Pod: pipeline-runner (transport)"]
        ...
        PR["Portal reconciler"] -->|PUT / POST / DELETE /streams/{project}.{pipeline}| Process
    end
...
- **Dynamic Stream Reloading:** Bento runs `-w -r /streams/resources.yaml streams /streams/{name}…`, one argument per seeded stream file (`values/runner/base-values.yaml.gotmpl`); `-r` loads the shared resources, `-w` reloads a changed file. The Portal reconciler creates, replaces or deletes each approved stream over the runner's REST API (`PUT`, then `POST` on a `404`, and `DELETE` on `/streams/{id}`), so a stream starts, restarts or stops without a pod restart and without disturbing its siblings.
- **Stream id:** the Portal names a pipeline's stream `{project}.{pipeline}`, and its expiry sweep `{project}.{pipeline}.expiry` (§8). A project name and a pipeline name are DNS labels with no dot, so one runner that serves several projects, as dev's does, holds one stream per project even when two projects name a pipeline alike, and a project cannot replace, stop or read the counters of another project's stream by choosing its name. The runner's counters carry the same id in their `stream` label, and the Portal reads a pipeline's counters (the metrics route, `NothingWritten`, `Stalled`) under that id only. A stream the runner still holds under a bare pipeline name, the id the Portal used before, is deleted once when the Portal first lists the runner for that project (T-3003).
